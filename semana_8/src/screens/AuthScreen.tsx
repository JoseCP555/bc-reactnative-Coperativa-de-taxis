...
export function AuthScreen(): React.JSX.Element {
  ...
  const loginForm = useForm<LoginForm>({ resolver: zodResolver(loginSchema), defaultValues: { username: 'Josher', password: 'David_JCP0' } });
  ...
  async function submitLogin(values: LoginForm): Promise<void> {
    ...
    catch { Alert.alert('No se pudo ingresar', 'Revisa tu usuario, contraseña y conexión. La cuenta de práctica está en esta pantalla.'); }
  }
    ...
    <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.panel}><Text style={styles.panelTitle}>{creatingAccount ? 'Crear cuenta de prueba' : 'Iniciar sesión'}</Text>
        {creatingAccount ? <>
          ...
          <View style={styles.demo}><Text style={styles.demoTitle}>Cuenta de práctica del profesor</Text><Text style={styles.demoText}>Usuario: Josher</Text><Text style={styles.demoText}>Contraseña: David_JCP0</Text></View>
          <OAuthPkceButton />
        </>}
...
const styles = StyleSheet.create({ flex: { flex: 1 }, safe: { flex: 1, backgroundColor: '#0d1117' }, wrap: { flexGrow: 1, justifyContent: 'center', padding: 22, maxWidth: 560, width: '100%', alignSelf: 'center' }, brand: { alignItems: 'center', marginBottom: 22 }, logo: { width: 54, height: 54, borderRadius: 17, backgroundColor: '#238636', alignItems: 'center', justifyContent: 'center' }, logoText: { color: '#fff', fontWeight: '900', fontSize: 20 }, title: { color: '#fff', fontWeight: '900', fontSize: 24, marginTop: 13, textAlign: 'center' }, subtitle: { color: '#9aa4af', marginTop: 5, textAlign: 'center' }, panel: { backgroundColor: '#111820', padding: 20, borderRadius: 18, borderWidth: 1, borderColor: '#26313a' }, panelTitle: { color: '#fff', fontSize: 21, fontWeight: '800', marginBottom: 14 }, demo: { marginTop: 14, padding: 12, borderRadius: 10, backgroundColor: '#0d1117' }, demoTitle: { color: '#61dafb', fontWeight: '800', marginBottom: 4 }, demoText: { color: '#bdc6d0', fontSize: 13, marginTop: 2 }, switch: { color: '#61dafb', textAlign: 'center', paddingTop: 15, fontWeight: '700' }, note: { color: '#687581', fontSize: 12, textAlign: 'center', lineHeight: 18, marginTop: 16 } });
